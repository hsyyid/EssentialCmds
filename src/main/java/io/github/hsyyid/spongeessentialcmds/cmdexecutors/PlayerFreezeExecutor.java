...
public class PlayerFreezeExecutor implements CommandExecutor
{
	public CommandResult execute(CommandSource src, CommandContext ctx) throws CommandException
	{
		...
		if(Main.frozenPlayers.contains(targetPlayer.getUniqueId()))
		{
			Main.frozenPlayers.remove(targetPlayer.getUniqueId());
			src.sendMessage(Texts.of(TextColors.GREEN, "Success! ", TextColors.YELLOW, "Un-froze player."));
		}
		else
		{
			Main.frozenPlayers.add(targetPlayer.getUniqueId());
